...
def test_export(fs):
    fs.create_file("/sys/class/gpio/export")
    export(19)
...
def test_unexport(fs):
    fs.create_file("/sys/class/gpio/unexport")
    unexport(26)
...
def test_direction(fs, test_input, expected):
    fs.create_file("/sys/class/gpio/gpio198/direction")
...
def test_input(fs, test_input, expected):
    fs.create_file("/sys/class/gpio/gpio0/value")
...
def test_output(fs, test_input, expected):
    fs.create_file("/sys/class/gpio/gpio15/value")
...
def test_edge(fs, test_input, expected):
    fs.create_file("/sys/class/gpio/gpio5/edge")
    edge(5, test_input)
